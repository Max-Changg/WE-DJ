...
const generateTransitions = (sourceSong: Song): TransitionSong[] => {
  const allSongs: Song[] = [
    ...
    { id: "t9", title: "Wake Me Up", artist: "Avicii", bpm: 124, key: "Bm" },
    {
      id: "t10",
      title: "Lean On",
    ...
    { id: "t12", title: "Midnight City", artist: "M83", bpm: 104, key: "Bb" },
  ];
    ...
    .map((song) => {
      ...
      if (sourceSong.key && song.key) {
        if (sourceSong.key === song.key) {
          ...
          if (
            harmonicKeys[sourceSong.key as keyof typeof harmonicKeys]?.includes(
              song.key
            )
          ) {
  ...
  return (
    <div className={cn("w-full space-y-6", className)}>
      ...
      <div className="max-h-96 overflow-y-auto space-y-4 pr-2">
        {transitions.slice(0, 10).map((song, index) => (
          <Card
            key={song.id}
